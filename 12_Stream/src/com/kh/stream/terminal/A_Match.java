...
//		a.method1();
	...
	public void method2() {
		...
		// 나이가 10살 이상인 학생들이 모두 여자인지 확인 - allMatch
		result = students.stream().filter(student -> student.getAge()>=10).allMatch(student -> student.getGender().equals("여자"));
		System.out.println(result);
		
		
		// 남학생들 중에 평균이 70점 이상인 학생이 한명이라도 존재하는지 확인 - anyMatch
		result = students.stream().anyMatch(student -> student.getGender().equals("남자") && (student.getMath()+ student.getEnglish()) / 2 >= 70);
		System.out.println(result);
	}
